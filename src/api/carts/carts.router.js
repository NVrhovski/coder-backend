...
cartsRouter.put('/:cid', async (req, res) => {
    try {
        const message = await cartManager.editCartProducts(req.params.cid, req.body.newProducts);
        return res.status(200).json({status: 'Success', payload: message.products})
    } catch (error) {
        return res.status(400).json({status: 'Error', error})
    }
...
cartsRouter.delete('/:cid', async (req, res) => {
    try {
        const message = await cartManager.deleteProductsInCart(req.params.cid);
        return res.status(200).json({status: 'Success', payload: message})
    } catch (error) {
        return res.status(400).json({status: 'Error', error})
    }
...
cartsRouter.put('/:cid/product/:pid', async (req, res) => {
    try {
        let oldCart = await cartManager.getCartProducts(req.params.cid);
        const message = await cartManager.editProductInCart(req.params.cid, req.params.pid, req.body.quantity, oldCart.products);
        return res.status(200).json({status: 'Success', payload: message})
    } catch (error) {
        return res.status(400).json({status: 'Error', error})
    }
...
cartsRouter.delete('/:cid/product/:pid', async (req, res) => {
    try {
        let oldCart = await cartManager.getCartProducts(req.params.cid);
        const message = await cartManager.removeProductFromCart(req.params.cid, req.params.pid, oldCart.products);
        return res.status(200).json({status: 'Success', payload: message})
    } catch (error) {
        return res.status(400).json({status: 'Error', error})
    }
